...
<%--以上是每个页面都有的内容--%>
<form action="${pageContext.request.contextPath}/complaint" method="post" name="complaint">
    <h3>意见反馈</h3><br/>
    ...
    <table border="3">
        ...
        <th>发布者id</th>
        <th>回复状态</th>
        <c:forEach items="${complaint}" var="p">
            <tr>
                <td align="center">${p.cp_time}</td>
                <td align="center">${p.cp_content}</td>
                <td align="center">${p.cp_a_id}</td>
                <td align="center">${p.cp_status}</td>
            </tr>
        </c:forEach>
    </table>
